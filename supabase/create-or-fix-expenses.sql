-- 経費管理システム - 包括的なセットアップスクリプト
-- 実行日: 2025-12-18
-- このスクリプトは既存のテーブルを考慮して安全に実行できます
...
-- user_idカラムが存在しない場合は追加（既存テーブル対応）
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE expense_audit_log ADD COLUMN user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE;
  END IF;
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '経費管理システムのセットアップが正常に完了しました';
  RAISE NOTICE 'テーブル: expenses, expense_audit_log';
  RAISE NOTICE 'RLSポリシー: 有効';
  RAISE NOTICE 'トリガー: 有効';
END $$;
